        ...
        <div class="header">
            @php
                ...
                $logo = $settings->get('email_logo', '')
                    ?: $settings->get('logo_main', '')
                    ?: $settings->get('site_logo', 'simple_web_ui/logo.png');
                $logoUrl = \App\Support\PublicStorageUrl::fromPath($logo) ?? asset('simple_web_ui/logo.png');

                $normalizedLogoPath = ltrim(str_replace('\\', '/', $logo), '/');
                ...
                if (isset($message) && ($message instanceof \Illuminate\Mail\Message) && $logoAbsolutePath && is_readable($logoAbsolutePath)) {
                    try {
                        $logoUrl = $message->embed($logoAbsolutePath);
                    } catch (\Throwable $exception) {
                        // Keep URL fallback when embedding fails.
                    }
                }
            @endphp
