            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('event_title')
                    ...
                    ->description(function ($record) {
                        $desc = $record->guest_name;
                        if (str_contains($record->id, 'arrival_p_')) {
                            $options = DB::table('booking_package_options')
                                ->join('package_options', 'booking_package_options.package_option_id', '=', 'package_options.id')
                                ->where('booking_id', $record->booking_id)
                                ->pluck('package_options.name')
                                ->toArray();
                            if (!empty($options)) {
