...
async function loadPackageDetails(slug) {
  try {
    ...
    // Try slug-based endpoint first (now supports both slug and numeric ID)
    try {
      const apiUrl = `/api/packages/${encodeURIComponent(slug)}`;
      const response = await fetch(apiUrl);
      
      if (response.ok) {
        packageData = await response.json();
      } else if (response.status === 404) {
        // Try fallback list lookup before showing not-found
      } else {
...
function renderPackagePage(pkg) {
  ...
  const safePackage = {
    ...pkg,
    itineraries: Array.isArray(pkg?.itineraries) ? pkg.itineraries : [],
    options: Array.isArray(pkg?.options) ? pkg.options : [],
    gallery_media: Array.isArray(pkg?.gallery_media) ? pkg.gallery_media : [],
    media: Array.isArray(pkg?.media) ? pkg.media : [],
  };

  if (safePackage.itineraries.length > 0) {
    renderItinerarySections(safePackage);
  } else {
    renderEmptyItineraryState();
  }

  // Render accommodation (keep showing for now)
  renderAccommodationSection();

  // Render What's Included
  renderIncludes(safePackage);

  // Set default dates for form
  setDefaultDates();
}

// NEW: Render all itineraries from API data
function renderItinerarySections(pkg) {
  ...
  const day3Container = document.getElementById('day3Container');
  const finalDayContainer = document.getElementById('finalDayContainer');
  ...
  pkg.itineraries.forEach((itinerary, index) => {
    ...
    if (!container) return; // Skip if no container for this day
    
    const hasOptions = (index === 0) && pkg.options && pkg.options.length > 0;
    const imgUrl = itinerary.image_url || day2LakeImage; // Fallback image
    
    let optionsHTML = '';
    if (hasOptions) {
      optionsHTML = `
        <div class="day1-options-box mt-4">
          <h5 class="mb-3">⚡ Choose Your Day ${itinerary.day_no} Adventures</h5>
          <p class="text-muted-soft mb-4">Select any <strong>two</strong> activities:</p>
...
function renderEmptyItineraryState() {
  const containers = [
    document.getElementById('day1Container'),
    document.getElementById('day2Container'),
    document.getElementById('day3Container'),
    document.getElementById('finalDayContainer'),
  ];
  ...
  const day1Container = document.getElementById('day1Container');
  if (!day1Container) return;

  day1Container.innerHTML = `
    <div class="timeline-item">
      <div class="timeline-dot">i</div>
      <div class="timeline-content">
        <div class="timeline-day">Itinerary</div>
        <div class="timeline-title">Itinerary coming soon</div>
        <div class="timeline-desc">Our team is preparing the detailed day-by-day plan for this package.</div>
      </div>
    </div>
  `;
}
...
function renderIncludes(pkg) {
  ...
  const includes = normalizeIncludes(pkg.includes);

  if (includes.length === 0) {
    container.innerHTML = `
      <li class="col-12">Package inclusions will be updated soon.</li>
    `;
    return;
  }
...
function normalizeIncludes(includes) {
  ...
  if (Array.isArray(includes)) {
    return includes.filter(Boolean).map(item => String(item).trim()).filter(item => item.length > 0);
  }
  ...
  if (typeof includes === 'string') {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        return parsed.filter(Boolean).map(item => String(item).trim()).filter(item => item.length > 0);
      }
    } catch (_) {
...
function showErrorState(msg) {
  ...
  if (errorMessage) {
    errorMessage.textContent = msg || "Sorry, we couldn't find the package you're looking for.";
  }
}
